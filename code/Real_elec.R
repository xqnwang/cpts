...
methods <- c("ARIMA", "MSCP", "MWCP", "MACP", "MPI", "MPID", "AcMCP")
cols <- c(
  "ARIMA" = "black", "MSCP" = "yellow", "MWCP" = "orange", "MACP" = "green",
  "MPI" = "blue", "MPID" = "purple", "AcMCP" = "red"
)
linetypes <- c(
  "ARIMA" = "solid", "MSCP" = "dashed", "MWCP" = "twodash", "MACP" = "dotted",
...
P_elec_info <- do.call(bind_rows, info) |>
  mutate(
...
saveRDS(P_elec_info, file = "result/P_elec_info.rds")

#--------------------------
# Plots: Coverage and width
cov_plot <- cov |>
  filter(index >= min(index) + calc_window) |>
...
P_elec_timeplot <- tp_data |>
  ggplot(aes(x = time)) +
  geom_line(aes(y = x, colour = "Observation"), size = 1.5) +
