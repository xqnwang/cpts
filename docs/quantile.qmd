...
2.  In order to satisfy R2, the sample size needs adjustments. Consider using **Kish's effective sample size** given by
...
-   **Pros:** robust, not so efficient.
    ...
    So, instead of Beta distribution, uniform distribution is used to define the required PDF and CDF.
...
The weighted quantiles now match the unweighted quantiles much more closely.
...
x <- c(0, 1, 100)
wE <- c(1, 1, 1)
wA <- c(1, 0, 1); non_zero <- wA != 0
wB <- c(1, 0.00001, 1)

# R1: consistency with existing quantile estimators
sapply(1:9, function(type){
  c(quantile_equal = quantile(x, probs = 0.5, type),
    survey = weighted.quantile.qrule(x, w = wE, p = 0.5, type),
    ggdist = ggdist::weighted_quantile(x, weights = wE, probs = 0.5, type))
}, simplify = TRUE, USE.NAMES = TRUE) |> t()

# R2: zero weight support
sapply(1:9, function(type){
  c(survey = weighted.quantile.qrule(x, w = wA, p = 0.5, type),
    survey_nozero = weighted.quantile.qrule(x[non_zero], w = wA[non_zero], p = 0.5, type),
    ggdist = ggdist::weighted_quantile(x, weights = wA, probs = 0.5, type),
    ggdist_nozero = ggdist::weighted_quantile(x[non_zero], weights = wA[non_zero], probs = 0.5, type))
}, simplify = TRUE, USE.NAMES = TRUE) |> t()

# R3: stability
sapply(1:9, function(type){
  c(survey_wA = weighted.quantile.qrule(x, w = wA, p = 0.5, type),
    survey_wB = weighted.quantile.qrule(x, w = wB, p = 0.5, type),
    ggdist_wA = ggdist::weighted_quantile(x, weights = wA, probs = 0.5, type),
    ggdist_wB = ggdist::weighted_quantile(x, weights = wB, probs = 0.5, type))
}, simplify = TRUE, USE.NAMES = TRUE) |> t()
```
...
-   To satisfy the property that $\hat{Q}_{X,i}(0.5)$ is equal to the sample median, we may suggest using **Kish\'s effective sample size** to create flat regions and make the output quantiles match the unweighted quantiles more closely.
...
<!-- -->
```
-   **Type8** may be a good choice as it gives median-unbiased estimates in the equal weight settings.
