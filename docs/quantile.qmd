...
sapply(1:9, function(type){
  c(quantile_equal = quantile(x, probs = p, type),
...
# R2: zero weight support
sapply(1:9, function(type){
  c(survey = weighted.quantile.qrule(x, w = wA, p = p, type),
    survey_nozero = weighted.quantile.qrule(x[non_zero], w = wA[non_zero], p = p, type),
    ggdist = ggdist::weighted_quantile(x, weights = wA, probs = p, type),
    ggdist_nozero = ggdist::weighted_quantile(x[non_zero], weights = wA[non_zero],
                                              probs = p, type),
    ggdist_kess = ggdist::weighted_quantile(x, weights = wA, probs = p, type, n = kess),
    ggdist_nozero_kess = ggdist::weighted_quantile(x[non_zero], weights = wA[non_zero],
                                                   probs = p, type, n = kess))
}, simplify = TRUE, USE.NAMES = TRUE) |> t()
...
p <- 0.5
sapply(1:9, function(type){
  c(survey_wA = weighted.quantile.qrule(x, w = wA, p = p, type),
