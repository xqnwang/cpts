...
$$
q_{t+1}=\underbrace{\eta g_t}_{\mathrm{P}}+\underbrace{r_t\left(\sum_{i=1}^t g_i\right)}_{\mathrm{I}}+\underbrace{g_t^{\prime}}_{\mathrm{D}},
$$
...
-   The goal is to estimate the distribution at the tail of a time series, which reflects the latest state of the underlying system.
...
-   **cons:** not robust (its breakdown point is zero).
    ...
    where $\alpha^*=\left(n^*+1\right) p$, $\beta^*=\left(n^*+1\right)(1-p)$, $t_i^*=s_i(\bar{\mathbf{w}})$ is the partial sum of normalized weight coefficients, $I_{t_i^*}\left(\alpha^*, \beta^*\right)$ is the CDF of the beta distribution $\text{Beta}(\alpha, \beta)$. $W_{\mathrm{HD}, i}^*$ can be considered as probabilities of observing the target quantile at the given position. See details from Examples 7 and 8.
