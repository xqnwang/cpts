...
#### The effect of constants in integrator

The integrator used in the paper is $r_t(x)=K_{\mathrm{I}} \tan \left(x \log (t) /\left(t C_{\mathrm{sat}}\right)\right)$, where $C_{\mathrm{sat}}$ controls how long the guaranteed coverage will be achieved, and $K_{\mathrm{I}}$ controls the change scale of the integrator.

Here we set the learning rate for the quantile tracking (P) to $\eta=0.1\hat{B}_t$.

1.  The initial wrong setting, $C_{\mathrm{sat}}=1$ and $K_{\mathrm{I}}=200$, in @fig-ar2roll-wrongPIDsetting gives a **wide variation** in the width of the prediction intervals, thus widening the mean and median values.

2.  The heuristic setting, $C_{\mathrm{sat}}=0.5618745$ and $K_{\mathrm{I}}=2$, in @fig-ar2roll makes the variation in the width of the prediction intervals more reasonable.

    However, upon comparison with other CP methods, it is evident that the relative situation of prediction interval width does not align with the relative situation of coverage.
...
To investigate the effect of P, I, and D and identify the source of large variations in prediction interval width, we present boxplots illustrating the width of ACP, P, PI, and PID. A learning rate of $0.005$ is applied to ACP, while for P, PI, and PID, the learning rates are set to 0.1B and 0.01B in @fig-ar2roll-wrongPIDsetting-boxplot-01 and @fig-ar2roll-wrongPIDsetting-boxplot-001, respectively.

1.  **ACP VS. P**
    -   ACP can be considered as a special case of the quantile tracker. The choice of learning rate is crucial for both methods. A higher learning rate results in a larger variation in width. Particularly, ACP may ouput infinite or null sets when a large learning rate is used.
2.  **P, PI and PID**
    -   The integrator appears to have minimal impact on the outcomes. Nevertheless, @angelopoulos2023 claims that it does nicely for most of the time series. We may need to do further exploration using time series with trend and seasonal patterns.

    -   The substantial variations stem from the scorecaster, it introduces variance into the score sequence that the sets will degrade in stability. Thus, scorecasters should be designed with care, as suggested by @angelopoulos2023. Their usage is advisable primarily in instances where trends or other patterns show in the score sequence.

    -   @fig-ar2roll-PID01 and @fig-ar2roll-PID001 present results of PID using the learning rate 0.1B and 0.01B respectively.

#### Conclusion

-   The choice of **learning rate** is crucial for ACP and PID.

-   The **constants in the integrator** should be chosen through heuristics or fine-tuned during a burn-in period. Further work is needed to validate the usefulness of the integrator.

-   The **scorecaster** should be designed with care, sometimes it can be harmful.
...
WPID_len <- data.frame(Method = "WPID", Width = len.update$PID)
WPID_cov <- mean(cov.update$PID)
load("../data/AR2_update.rda")
ACP_len <- data.frame(Method = "ACP", Width = len.update$ACP)
ACP_cov <- mean(cov.update$ACP)
load("../data/AR2_PID_01.rda")
PID_len <- data.frame(Method = "PID", Width = len.PID$PID)
PID_cov <- mean(cov.PID$PID)
PI_len <- data.frame(Method = "PI", Width = len.PID$PI)
PI_cov <- mean(cov.PID$PI)
P_len <- data.frame(Method = "P", Width = len.PID$P)
P_cov <- mean(cov.PID$P)
len <- rbind(ACP_len, PID_len, PI_len, P_len, WPID_len)
ggplot(len, aes(x = Method, y = Width)) +
  geom_boxplot() +
  scale_x_discrete(breaks=c("ACP","P","PI", "PID", "WPID"),
                   labels=c(paste("ACP", ACP_cov), paste("P", P_cov),
                            paste("PI", PI_cov), paste("PID", PID_cov),
                            paste("WPID", WPID_cov))) +
  theme_bw()
```
...
ggplot(len, aes(x = Method, y = Width)) +
  geom_boxplot() +
  scale_x_discrete(breaks=c("ACP","P","PI", "PID"),
                   labels=c(paste("ACP", ACP_cov), paste("P", P_cov),
                            paste("PI", PI_cov), paste("PID", PID_cov))
                   ) +
  theme_bw()
...
#| fig-width: 10
#| fig-height: 9
#| fig-cap: Local coverage frequencies and width for PID with rolling training set and calibration set (k=500). The learning rate is set to 0.1B.

load("../data/AR2_PID_plot_01.rda")
...
#| fig-width: 10
#| fig-height: 9
#| fig-cap: Local coverage frequencies and width for PID with rolling training set and calibration set (k=500). The learning rate is set to 0.01B.

load("../data/AR2_PID_plot_001.rda")
ggarrange(lcl.PID, lcl.PI, lcl.P,
          mw.PID, mw.PI, mw.P,
          mdw.PID, mdw.PI, mdw.P,
          box.PID, box.PI, box.P,
...
-   [ ] Generalize the existing methods and get a general function for implementation.
    ...
    -   [ ] Include weights in PID (integrator?).
    -   [ ] Handle seasonal time series and specify methods used for forecasting and scorecasting.
-   [ ] Multi-step ahead forecasting.
    -   [ ] Correlation between $h$-step-ahead residuals.
    -   [ ] Correlation between $s$-step-ahead residuals and $h$-step-ahead residuals.
