...
axis(1, at = c(1, tr, tr + tc, T), labels = c("1", "$t_r$", "$t_r+t_c$", "$T$"))
for (i in seq(tc)) {
...
tikzCoord(T * 1.1, tr + 0.85 * tc, "yhat")
tikzAnnotate("\\draw[decorate] (yhat) -- node {\\small $\\{\\hat{y}_{t+h|t}\\}_{t_r \\le t \\le t_r+t_c-h}$} (yhat);")
tikzCoord(T * 1.1, tr + 0.35 * tc, "shat")
tikzAnnotate("\\draw[decorate] (shat) -- node {\\small $\\{\\hat{s}_{t+h|t}\\}_{t_r \\le t \\le t_r+t_c-h}$} (shat);")
tikzCoord(T * 1.15, tr - 0.5 * tc, "Chat")
...
tikzCoord(0.9 * T, tr + 2, "bottombrace")
tikzAnnotate(c(
...
Arrows(T * 1.05, tr + 0.75 * tc, T * 1.05, tr + 0.55 * tc,
  lwd = 3, arr.lwd = 3, arr.length = 0.2, arr.width = 0.1)
Arrows(T * 1.05, tr + 0.2 * tc, T * 1.05, tr - 0.15 * tc,
  lwd = 3, arr.lwd = 3, arr.length = 0.2, arr.width = 0.1)
tikzCoord(tr+tc+3*h, tr, "yhat2")
