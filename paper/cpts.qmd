...
$$
where $\theta_{i}$ is a complex combination of $\phi$ terms from the previous $h-1$ equations.\todo{Too vague} So we conclude that the $h$-step-ahead forecast error sequence $\{e_{t+h|t}\}$ follows an approximate MA$(h-1)$ process.
:::
...
&= \hat{y}_{t+h|t}+e_{t+h|t},
\end{align*}
Therefore, the forecast errors of optimal $h$-step-ahead forecasts follow an approximate AR($p$) process, where $p=\min\{d, h-1\}$.
<!--This implies that, the optimal $h$-step-ahead forecast errors are at most an approximate AR($h-1$) process.-->
:::
