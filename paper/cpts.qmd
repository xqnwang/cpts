...
-   multi-step, univariate time series forecasting; online learning and split cp framework (A recent line of work develops online conformal prediction methods for the setting where the data arrives in a sequential order)

-   generalization of existing methods (easy-to-use)

-   properties of multi-step forecast errors in a general non-stationary autoregressive process, thus AcMCP, theoretical long-run coverage guarantee without any assumption on distributional shift

-   show experimentally that

Paper structure.

In the following sections, we first introduce the setup for time series forecasting problems, and then generalize some existing conformal prediction methods to deal with multi-step time series forecasting. Finally, we explore the properties of forecast errors for optimal multi-step forecasts, and then propose the **autocorrelated multi-step conformal prediction** (AcMCP) method to account for the serial correlation of multi-step forecast errors.
