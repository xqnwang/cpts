...
% Authors
  ...
  \addresses{%
   ...
   \lfoot{\sf Wang, Hyndman: 8 May 2024}
...
For \(T=h+1\),
\(E_T = \mathrm{err}_{h+1|1}-\alpha \leq 1 \leq cg(T-h) + h\) as
\(c>0\), \(h\geq 1\), and \(g\) is nonnegative. Thus,
Equation~\ref{eq-cov_acmcp} holds for \(T=h+1\).
...
Therefore, assuming Equation~\ref{eq-cov_acmcp} holds true up to
\(T-h\), we can deduce the desired outcome at \(T\). This completes the
proof for the first part of Proposition~\ref{prp-cov_acmcp}.
