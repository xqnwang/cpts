...
\nocover
  ...
  \addresses{%
   ...
   \lfoot{\sf Wang, Hyndman: 3 October 2024}
...
through a sequential adjustment
process\todo{How are $\alpha_{t+h|t}^{*}$ and $\alpha_{t+h|t}$ related?}
...
intervals.\todo{Why "however"? Isn't this what we want?}
...
\textcite{sommer2023}. \todo[inline]{Only approximately}
...
AR(\(p\)) model for the forecast error \(e_{t+j|t}\) and here the AR
model is applied at the forecast horizon \(j\), rather than at the time
index \(t\). \todo[inline]{Define $\phi_i^{(j)}$}
...
the previous \(h-1\) equations.\todo{Too vague} So we conclude that the
\(h\)-step-ahead forecast error sequence \(\{e_{t+h|t}\}\) follows an
MA\((h-1)\) process.
\end{proof}
...
errors that are white noise.\todo{Again, only approximately.}
...
forecast errors are at most an approximate AR(\(h-1\)) process.
\end{proof}
