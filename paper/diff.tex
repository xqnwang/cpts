...
%DIF DEL /dev/fd/11       Wed Jan 28 21:08:14 2026
%DIF ADD paper/cpts.tex   Wed Jan 28 21:02:45 2026
%DIF 2a2
...
\newtheorem{corollary}{Corollary}[section] %DIF > 
%DIF -------
\theoremstyle{remark}
...
forecasting}
\author{
Xiaoqian Wang\DIFdelbegin %DIFDELCMD < \thanks{Corresponding author. Xiaoqian Wang, Department of
%DIFDELCMD < Econometrics \& Business Statistics, Monash University, Clayton VIC,
%DIFDELCMD < 3800, Australia. E-mail address:
%DIFDELCMD < \href{mailto:xiaoqian.wang@monash.edu}{xiaoqian.wang@monash.edu} (X.
%DIFDELCMD < Wang).} %%%
\DIFdelend \DIFaddbegin \thanks{Corresponding author. Xiaoqian Wang, Academy of
...
intervals. This method \DIFdelbegin \DIFdel{ensures theoretical long-run coverage guarantees }\DIFdelend \DIFaddbegin \DIFadd{guarantees asymptotic marginal coverage }\DIFaddend for
multi-step prediction intervals, though we note that\DIFdelbegin \DIFdel{increased
forecasting horizons may exacerbate deviations from the target coverage ,
particularly in the context of limited sample sizes}\DIFdelend \DIFaddbegin \DIFadd{, for finite
samples, the coverage error admits an upper bound that increases with
the forecasting horizon}\DIFaddend . Additionally, we extend several
easy-to-implement conformal prediction methods, originally designed for
single-step forecasting, to accommodate multi-step scenarios. Through
empirical evaluations, including simulations and applications to data,
we demonstrate that AcMCP achieves coverage that closely aligns with the
target within local windows, while providing adaptive prediction
intervals that effectively respond to varying conditions.
\end{abstract}
...
\DIFaddbegin \renewcommand{\thecorollary}{\arabic{corollary}} %DIF >  numbering corollaries sequentially and individually
\setcounter{corollary}{0}

\DIFaddend \section{Introduction}\label{sec-intro}

Conformal prediction \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{vovk2005} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{papadopoulos2002, vovk2005} }\hskip0pt%DIFAUXCMD
}\DIFaddend is a powerful
...
model \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{shafer2008, papadopoulos2008, barber2021} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{papadopoulos2007, papadopoulos2008, shafer2008, barber2021}
}\hskip0pt%DIFAUXCMD
}\DIFaddend or non-parametric model \citep{lei2014}.
...
the cost of increased computational complexity. \DIFaddbegin \DIFadd{Both split and full
conformal prediction methods guarantee coverage at the target level
under the assumption of data exchangeability. }\DIFaddend Jackknife+ strikes a
balance between these methods, offering a compromise between statistical
precision and computational demands. \DIFdelbegin \DIFdel{All three methods guarantee
coverage at the target level under the assumption of data
exchangeability .
}\DIFdelend \DIFaddbegin \DIFadd{Under assumptions of algorithmic
...
\mbox{%DIFAUXCMD
\citet{gupta2022} }\hskip0pt%DIFAUXCMD
further generalize jackknife+ and related out-of-bag
...
\citep{gibbs2021, zaffran2022, bastani2022}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{gibbs2021, gibbs2024, zaffran2022, bastani2022}}\hskip0pt%DIFAUXCMD
}\DIFaddend , label shift
...
\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{oliveira2024, xu2021, xu2023}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{chernozhukov2018, oliveira2024, xu2021, xu2023}}\hskip0pt%DIFAUXCMD
...
prediction methods \DIFdelbegin \DIFdel{under classical nonparametric assumptions to achieve
asymptotic }\DIFdelend \DIFaddbegin \DIFadd{to time series settings and establish asymptotically
}\DIFaddend valid conditional coverage \DIFdelbegin \DIFdel{for time series}\DIFdelend \DIFaddbegin \DIFadd{under the assumption that model errors are
stationary and strongly mixing}\DIFaddend . \citet{barber2023} use weighted residual
...
conformal inference \DIFaddbegin \DIFadd{(ACI, denoted as ACP hereafter) }\DIFaddend in an online manner
...
based on historical miscoverage frequencies. \DIFaddbegin \DIFadd{However, ACP may yield
infinite or empty prediction intervals when the \(\alpha\) drifts below
\(0\) or exceeds \(1\), respectively. }\DIFaddend Follow-up work has refined this
...
\citet{zaffran2022}, and \citet{gibbs2024}. \DIFdelbegin \DIFdel{However, these methods may produce prediction
intervals that are either infinite or null. To address this issue,
recent }\DIFdelend \DIFaddbegin \DIFadd{Recent }\DIFaddend research has proposed
a generalized updating process that tracks the quantile of the
nonconformity score sequence, \DIFaddbegin \DIFadd{rather than the miscoverage rate, }\DIFaddend as
...
single-step forecasting\DIFdelbegin \DIFdel{. However, }\DIFdelend \DIFaddbegin \DIFadd{, even though }\DIFaddend many applications require \DIFdelbegin \DIFdel{forecasts
for multiple future time steps, not just one. Related research into
}\DIFdelend \DIFaddbegin \DIFadd{reliable
uncertainty quantification over multiple future horizons. The literature
on }\DIFaddend multi-step \DIFdelbegin \DIFdel{time series forecasting is limited and does not account for
the temporal dependencies inherent in }\DIFdelend \DIFaddbegin \DIFadd{conformal prediction is comparatively limited and, when it
does consider }\DIFaddend multi-step forecasts\DIFaddbegin \DIFadd{, it often reduces the problem to
\(H\) horizon-specific tasks by constructing separate prediction sets
for each \(y_{t+h}\), \(h\in[H]\), without considering how forecasts at
different horizons are related}\DIFaddend . For example, \citet{stankeviciute2021}
integrate conformal prediction with recurrent neural networks for
multi-step forecasting and then apply Bonferroni correction to achieve
the desired coverage rate. This approach, however, assumes data
independence, which is \DIFdelbegin \DIFdel{not }\DIFdelend often unrealistic for time series data.
\citet{yang2024ts} propose Bellman conformal inference, an extension of
adaptive conformal prediction, to control multi-step miscoverage rates
simultaneously at each time point \(t\) by minimizing a loss function
that balances the average interval length across forecast horizons with
miscoverage. While this method considers multi-step intervals, it does
not account for their temporal dependencies and may be computationally
intensive when solving the associated optimisation problems at each time
step. \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{lopes2024} }\hskip0pt%DIFAUXCMD
introduce ConForME, a method that focuses on
generating uniform prediction intervals across the entire forecast
horizon, aiming to ensure the overall validity of these intervals.
}\DIFaddend Additionally, several extensions to multivariate targets have been
explored, see, e.g., \citet{schlembach2022} and \citet{sun2022}.
...
histories. The \DIFdelbegin \DIFdel{distribution of
\(\{(\bm{x}_t, y_t)\}_{t \geq 1} \subseteq \mathbb{R}^p \times \mathbb{R}\)
}\DIFdelend \DIFaddbegin \DIFadd{joint distribution of \(\{(\bm{x}_t, y_t)\}_{t \geq 1}\),
where \(\bm{x}_t\in\mathbb{R}^p\) and \(y_t\in\mathbb{R}\), }\DIFaddend is allowed
to vary over time, \DIFdelbegin \DIFdel{to accommodate }\DIFdelend \DIFaddbegin \DIFadd{thereby accommodating }\DIFaddend non-stationary processes. At
...
expect \DIFdelbegin \DIFdel{the output \(\hat{\mathcal{C}}_{t+h|t}\) to be a
}\emph{\DIFdel{valid}} %DIFAUXCMD
\DIFdel{prediction interval so that \(y_{t+h}\) falls within the prediction interval \(\hat{\mathcal{C}}_{t+h|t}\) at least
\(100(1-\alpha)\%\) of the time}\DIFdelend \DIFaddbegin \DIFadd{to construct prediction intervals \(\hat{\mathcal{C}}_{t+h|t}\)
that achieve long-run coverage guarantees, in the sense that
\(\lim _{T \rightarrow \infty} \frac{1}{T} \sum_{t=1}^T\mathbf{1}\left\{y_{t+h} \in \hat{\mathcal{C}}_{t+h|t}\right\} \geq 1-\alpha\)}\DIFaddend .
...
originally designed for single-step forecasting to \DIFdelbegin \DIFdel{accommodate
}\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend multi-step
\DIFdelbegin \DIFdel{forecasting scenarios. }\DIFdelend \DIFaddbegin \DIFadd{setting by constructing horizon-specific prediction intervals. These
extensions follow the common practice of treating each horizon
independently and, in general, do not provide theoretical long-run
coverage guarantees for multi-step prediction intervals, with the
exception of the proposed multi-step conformal PID control (MPID)
method, which admits such a guarantee. }\DIFaddend Second, we provide theoretical
\DIFdelbegin \DIFdel{proofs
demonstrating that}\DIFdelend \DIFaddbegin \DIFadd{results showing that, under a general non-stationary autoregressive DGP,
}\DIFaddend the forecast errors of optimal \(h\)-step-ahead forecasts \DIFdelbegin \DIFdel{approximate }\DIFdelend \DIFaddbegin \DIFadd{can be well
approximated by }\DIFaddend a linear combination of at most its lag \((h-1)\) with
respect to forecast horizon\DIFdelbegin \DIFdel{when we assume a general non-stationary
autoregressive data generating process}\DIFdelend . Third, \DIFaddbegin \DIFadd{building on the theoretical results,
}\DIFaddend we introduce the autocorrelated multi-step conformal prediction (AcMCP)
method, which \DIFaddbegin \DIFadd{explicitly }\DIFaddend accounts for the autocorrelations of multi-step
forecast errors \DIFdelbegin \DIFdel{. Our
method }\DIFdelend \DIFaddbegin \DIFadd{at the calibration stage by preserving the dependence
structure among nonconformity scores. AcMCP }\DIFaddend is proven to \DIFdelbegin \DIFdel{achieve long-run coverage guarantees }\DIFdelend \DIFaddbegin \DIFadd{guarantee
asymptotic marginal coverage for multi-step prediction intervals, with
the coverage gap converging to zero as the sample size increases,
}\DIFaddend without making any assumptions on data distribution shifts. \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citet{lopes2024} }\hskip0pt%DIFAUXCMD
introduce
...
overall validity of these intervals. In contrast
}\DIFdelend \DIFaddbegin \DIFadd{In contrast
to \mbox{%DIFAUXCMD
\citet{lopes2024}}\hskip0pt%DIFAUXCMD
}\DIFaddend , our method targets pointwise prediction intervals
for each specific forecast horizon, \(h\in[H]\). \DIFdelbegin \DIFdel{We also highlight that for \(t \ll \infty\), increasing the
forecast horizon \(h\) generally leads to greater deviations from the
target coverage, which aligns with our expectations. }\DIFdelend Finally, we illustrate
the practical utility of these proposed methods through two simulations
and two applications to electricity demand and eating-out expenditure
forecasting. \DIFaddbegin \DIFadd{The results show that the proposed AcMCP method adapts
effectively to changes in the observed data distribution and, among the
proposed extensions that achieve coverage close to the target level,
generally yields more informative and narrower prediction intervals,
particularly for larger forecast horizons.
}\DIFaddend 

We developed the \texttt{conformalForecast} package for R \DIFdelbegin \DIFdel{, available at
}%DIFDELCMD < \url{https://github.com/xqnwang/conformalForecast}%%%
\DIFdel{, }\DIFdelend to implement
the proposed multi-step conformal prediction methods\DIFaddbegin \DIFadd{, the package is
publicly available on CRAN}\DIFaddend . All the data and code to reproduce the
...
\(\hat{f}_t\) trained using the historical data \(z_{1:t}\). \DIFaddbegin \DIFadd{The model
\(\hat{f}_t\) is updated over time and is re-trained whenever new
observations become available. }\DIFaddend Throughout the paper, we assume that the
predictors are known into the future. \DIFdelbegin \DIFdel{In
this way, we perform }\DIFdelend \DIFaddbegin \DIFadd{This corresponds to an }\DIFaddend ex-post
forecasting \DIFdelbegin \DIFdel{and there is no additional uncertainty
introduced }\DIFdelend \DIFaddbegin \DIFadd{setting, commonly adopted when predictors are deterministic,
policy-driven, or provided through external scenarios. By conditioning
on future predictors, we focus exclusively on the uncertainty in the
response variable \(y_t\) and avoid introducing additional uncertainty
}\DIFaddend from forecasting the exogenous \DIFdelbegin \DIFdel{predictors}\DIFdelend \DIFaddbegin \DIFadd{inputs}\DIFaddend . Using the forecasting model
\(\hat{f}_t\), we are able to produce \(H\)-step point forecasts,
\(\{\hat{y}_{t+h|t}\}_{h\in[H]}\), using the future values for the
predictors. We define the \emph{nonconformity score} as the (signed)
forecast error \[
s_{t+h|t}=\mathcal{S}(z_{1:t}, y_{t+h}):=y_{t+h}-\hat{f}_t(z_{1:t},\bm{x}\DIFdelbegin \DIFdel{_{t+1:h}}\DIFdelend \DIFaddbegin \DIFadd{_{(t+1):(t+h)}}\DIFaddend )=y_{t+h}-\hat{y}_{t+h|t}.
\] The task is to employ conformal inference to build \(H\)-step
prediction intervals,
\DIFdelbegin \DIFdel{\(\{\hat{\mathcal{C}}_{t+h|t}^{\alpha}(z_{1:t},\bm{x}_{t+1:h})\}_{h\in[H]}\)}\DIFdelend \DIFaddbegin \DIFadd{\(\{\hat{\mathcal{C}}_{t+h|t}^{\alpha}(z_{1:t},\bm{x}_{(t+1):(t+h)})\}_{h\in[H]}\)}\DIFaddend ,
...
in the data \DIFdelbegin \DIFdel{. Instead}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{zaffran2022}}\hskip0pt%DIFAUXCMD
. Following related work such as
\mbox{%DIFAUXCMD
\citet{wisniewski2020} }\hskip0pt%DIFAUXCMD
and \mbox{%DIFAUXCMD
\citet{zaffran2022}}\hskip0pt%DIFAUXCMD
}\DIFaddend , throughout the conformal
...
\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{papadopoulos2007}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{papadopoulos2002, vovk2005}}\hskip0pt%DIFAUXCMD
}\DIFaddend . With sequential splitting, multiple
...
learning framework\DIFaddbegin \DIFadd{, which is closely related to \mbox{%DIFAUXCMD
\citet{zaffran2022}}\hskip0pt%DIFAUXCMD
, }\DIFaddend for
...
separate conformal prediction procedure for each \(h \in [H]\). \DIFaddbegin \DIFadd{We note
that the extensions of existing approaches introduced in this section do
not provide theoretical long-run coverage guarantees, with the exception
of the multi-step conformal PID control (MPID) method, which admits such
a guarantee.
}\DIFaddend 
...
for building prediction intervals using a pre-trained model \DIFdelbegin \DIFdel{in
regression settings}\DIFdelend \DIFaddbegin \DIFadd{on a
training set}\DIFaddend . A key advantage of SCP is its ability to guarantee
coverage by assuming data exchangeability. \DIFaddbegin \DIFadd{In regression setting, the
standard SCP method randomly separates the available \(n\) data points,
\(Z_i = (X_i, Y_i) \in \mathbb{R}^d \times \mathbb{R}\),
\(i=1,\ldots,n\), into a proper training set of size \(n_t\) and a
calibration set of size \(n_c\). Given a regression model
\(\hat{\mu}: \mathbb{R}^d \rightarrow \mathbb{R}\) that is fitted on the
training set and a score function \(\mathcal{S}\), a nonconformity score
\(s_i = \mathcal{S}\left(X_i, Y_i\right)\) is computed on every data
point \(i\) in the calibration set to measure the nonconformity between
the calibration's response values and the predicted values obtained from
the fitted model \(\hat{\mu}\). Then SCP computes the prediction
interval for the test data \(Y_{n+1}\) using }\[
\DIFadd{\hat{\mathcal{C}}_{n+1}^{\alpha}\left(X_{n+1}\right) = \left\{y\in\mathbb{R}: \mathcal{S}\left(X_{n+1}, y\right) \leq Q_{1-\alpha}\left(\sum_{i \in \mathcal{D}_{\text{cal}}}\frac{1}{n_c+1}\cdot\delta_{s_{i}}+\frac{1}{n_c+1}\cdot\delta_{+\infty}\right)\right\},
}\] \DIFadd{where \(\mathrm{Q}_\tau(\cdot)\) denotes the \(\tau\)-quantile of its
argument, and \(\delta_a\) denotes the point mass at \(a\). }\DIFaddend Time series
...
\(h\)-step-ahead prediction intervals over time. \DIFdelbegin \DIFdel{Instead of assuming
exchangeability, }\DIFdelend MSCP applies conformal
...
a hypothesized test observation \(y\)\DIFdelbegin \DIFdel{, \(Q_\tau(\cdot)\) denotes the
...
is characterized by the total variation \DIFaddbegin \DIFadd{distance }\DIFaddend between the swapped
...
\DIFdelbegin \DIFdel{We }\DIFdelend \DIFaddbegin \DIFadd{For the subsequent empirical study, we }\DIFaddend choose fixed weights
...
Next we extend the adaptive conformal \DIFdelbegin \DIFdel{prediction (ACP}\DIFdelend \DIFaddbegin \DIFadd{inference (ACI, denoted as ACP
hereafter}\DIFaddend ) method proposed by \citet{gibbs2021} to address multi-step
...
event
\DIFdelbegin \DIFdel{\(\mathrm{err}_{t|t-h} = \mathbb{1}\left\{y_t \notin \hat{\mathcal{C}}_{t|t-h}^{\alpha_{t|t-h}}\right\}\)}\DIFdelend \DIFaddbegin \DIFadd{\(\mathrm{err}_{t|t-h} = \mathbf{1}\left\{y_t \notin \hat{\mathcal{C}}_{t|t-h}^{\alpha_{t|t-h}}\right\}\)}\DIFaddend .
...
sequence of candidate thresholds. \DIFdelbegin \DIFdel{However, both previous methods fail to promptly adapt to the local changes }\DIFdelend \DIFaddbegin \DIFadd{Both methods rely on update schemes
that place substantial weight on older historical data, which may impede
rapid adaptation to abrupt changes \mbox{%DIFAUXCMD
\citep{gibbs2024}}\hskip0pt%DIFAUXCMD
. Therefore,
\mbox{%DIFAUXCMD
\citet{gibbs2024} }\hskip0pt%DIFAUXCMD
propose an alternative expert selection scheme that
...
responses to sudden environmental shifts}\DIFaddend .
\DIFdelbegin \DIFdel{To address this limitation,
\mbox{%DIFAUXCMD
\citet{gibbs2024} }\hskip0pt%DIFAUXCMD
suggest adaptively tuning the step size parameter \(\gamma\) in an online setting, choosing an ``optimal'' value for
...
horizon \(h\in[H]\), the \DIFdelbegin \DIFdel{iteration }\DIFdelend \DIFaddbegin \DIFadd{estimated \(1-\alpha\) quantile }\DIFaddend of the
\(h\)-step-ahead \DIFdelbegin \DIFdel{quantile
estimate is given by }\DIFdelend \DIFaddbegin \DIFadd{score at time \(t\) is updated iteratively as
}\DIFaddend \begin{equation}\phantomsection\label{eq-mpid}{
...
\DIFdelbegin \DIFdel{Proposition~\ref{prp-cov_acmcp}}\DIFdelend \DIFaddbegin \DIFadd{Corollary~\ref{cor-cov_acmcp}}\DIFaddend .
...
by any suitable forecasting model (or ``scorecaster''\DIFaddbegin \DIFadd{, see
Section~\ref{sec-acmcp} for a detailed discussion}\DIFaddend ) trained using the
\(h\)-step-ahead nonconformity scores available up to and including time
\(t\). \DIFdelbegin \DIFdel{This module provides additional benefits only when the scorecaster is well-designed and there are }\DIFdelend \DIFaddbegin \DIFadd{The effectiveness of the D control depends on the ability of the
scorecaster to capture systematic and }\DIFaddend predictable patterns in the
nonconformity scores\DIFaddbegin \DIFadd{, such as temporal dependence or conditional
heteroskedasticity. Only when such structure is present and adequately
modeled, this module provides additional benefits}\DIFaddend ; otherwise, it may
increase variability in the coverage and prediction intervals.

\DIFaddbegin \DIFadd{The MPID method will be improved in Section~\ref{sec-acmcp} by proposing
the AcMCP method, in which the D control component of MPID is replaced
by a specific and structured approach to explicitly capture the
autocorrelations of multi-step forecast errors. As a consequence, the
long-run coverage guarantees established for AcMCP in
Section~\ref{sec-cov} also apply to the MPID method.
}

\DIFaddend \section{Autocorrelated multi-step conformal
...
errors \(e_{t+h|t}\) \DIFdelbegin \DIFdel{will be }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend correlated with the forecast errors from the
\DIFdelbegin \DIFdel{past }\DIFdelend \DIFaddbegin \DIFadd{previous }\DIFaddend \(h-1\) steps, as \DIFdelbegin \DIFdel{forecast errors accumulate }\DIFdelend \DIFaddbegin \DIFadd{errors propagate through the recursive
forecasting procedure }\DIFaddend over the forecast horizon. However, no conformal
...
their autocorrelations \DIFaddbegin \DIFadd{while providing theoretical long-run coverage
guarantees}\DIFaddend .
...
\citep{dahlhaus2012}}\hskip0pt%DIFAUXCMD
}\DIFaddend .
...
noise. \DIFaddbegin \DIFadd{The approximation is obtained in the proof via a first-order
Taylor series expansion of the forecasting function, where higher-order
remainder terms are neglected. }\DIFaddend Therefore, the optimal \(h\)-step-ahead
forecast errors are at most serially correlated to lag \((h-1)\).

\end{proposition}

\DIFaddbegin \begin{proof}
\DIFadd{See Section~\ref{sec-proof_ar}.
}\end{proof}

\DIFaddend Proposition~\ref{prp-ar} suggests that the optimal \(h\)-step ahead
...
be derived using Wold's representation theorem. \DIFdelbegin \DIFdel{Here, we extend the
investigation }\DIFdelend \DIFaddbegin \DIFadd{The statement can be
extended }\DIFaddend to time series generated by a general non-stationary
autoregressive process \DIFdelbegin \DIFdel{, and provide the corresponding proposition. The
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{sommer2023}}\hskip0pt%DIFAUXCMD
, as described in
Proposition~\ref{prp-ma}. We provide the }\DIFaddend proof of this proposition \DIFdelbegin \DIFdel{is provided }\DIFdelend in
Section~\ref{sec-proof_ma} \DIFaddbegin \DIFadd{based on Proposition~\ref{prp-ar}}\DIFaddend .
...
non-stationary autoregressive process as given in Equation
\eqref{eq-dgp}, and assume that any exogenous predictors are known into
the future. \DIFdelbegin \DIFdel{Then the }\DIFdelend \DIFaddbegin \DIFadd{Under the approximation in Proposition~\ref{prp-ar}, the
}\DIFaddend forecast errors of optimal \(h\)-step-ahead forecasts follow an
...
\DIFaddbegin \begin{proof}
\DIFadd{See Section~\ref{sec-proof_ma}.
}\end{proof}

\DIFaddend \subsection{The AcMCP method}\label{sec-novel}
...
iteration of the \(h\)-step-ahead quantile estimate \DIFaddbegin \DIFadd{for the score }\DIFaddend is
given by \begin{equation}\phantomsection\label{eq-acmcp}{
...
viewed as a further extension of the PID method \DIFaddbegin \DIFadd{by
\mbox{%DIFAUXCMD
\citet{angelopoulos2024}}\hskip0pt%DIFAUXCMD
}\DIFaddend . Nevertheless, AcMCP diverges from PID with
...
from past steps (i.e.~\(e_{t+h-1|t}, \ldots, e_{t+1|t}\)). \DIFdelbegin \DIFdel{Thus,
we
perform }\DIFdelend \DIFaddbegin \DIFadd{Unlike MPID,
which treats }\DIFaddend multi-step \DIFaddbegin \DIFadd{prediction problems independently across
horizons, AcMCP performs multi-step }\DIFaddend conformal prediction recursively,
\DIFdelbegin \DIFdel{contrasting with
the independent approach employed in MPID. Moreover, the inclusion }\DIFdelend \DIFaddbegin \DIFadd{explicitly accounting for the serial dependence among multi-step
forecast errors. Importantly, the role }\DIFaddend of \(\tilde{e}_{t+h|t}\) is not
\DIFdelbegin \DIFdel{intended }\DIFdelend to forecast the nonconformity scores \DIFdelbegin \DIFdel{(i.e., forecast errors), but rather to incorporate autocorrelations present in }\DIFdelend \DIFaddbegin \DIFadd{themselves, but to incorporate the
autocorrelation structure of }\DIFaddend multi-step forecast errors \DIFdelbegin \DIFdel{within the
resulting multi-step }\DIFdelend \DIFaddbegin \DIFadd{into the
construction of the resulting }\DIFaddend prediction intervals.

\subsection{Coverage guarantees}\DIFdelbegin %DIFDELCMD < \label{coverage-guarantees}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{sec-cov}
\DIFaddend 
...
\DIFaddbegin \begin{proof}
\DIFadd{See Section~\ref{sec-proof_cov_rt}.
}\end{proof}

\DIFadd{When \(h=1\), }\DIFaddend Proposition~\ref{prp-cov_rt} \DIFaddbegin \DIFadd{reduces to Proposition 2 of
\mbox{%DIFAUXCMD
\citet{angelopoulos2024}}\hskip0pt%DIFAUXCMD
. While \mbox{%DIFAUXCMD
\citet{angelopoulos2024} }\hskip0pt%DIFAUXCMD
only consider
one-step-ahead forecasting, Proposition~\ref{prp-cov_rt} in this paper
is more general and establishes an explicit upper bound on the coverage
gap for multi-step forecasting. Proposition~\ref{prp-cov_rt} }\DIFaddend indicates
that, for \DIFdelbegin \DIFdel{\(t \ll \infty\)}\DIFdelend \DIFaddbegin \DIFadd{\(T \ll \infty\)}\DIFaddend , increasing the forecast horizon \(h\) tends
to amplify \DIFaddbegin \DIFadd{in-sample }\DIFaddend deviations from the target coverage because
\DIFdelbegin \DIFdel{\(g(T-h)/(t-h)\) }\DIFdelend \DIFaddbegin \DIFadd{\(g(T-h)/(T-h)\) }\DIFaddend is non-increasing, given that the admissible function
\(g\) is sublinear, nonnegative, and nondecreasing. \DIFaddbegin \DIFadd{Importantly, this
effect reflects finite-sample behavior rather than a breakdown of the
asymptotic coverage guarantee. }\DIFaddend This is consistent with expectations, as
...
intervals may not scale perfectly \DIFaddbegin \DIFadd{in finite samples }\DIFaddend with the increasing
uncertainty, leading to a larger discrepancy between the desired and
actual coverage.
...
\DIFdelbegin %DIFDELCMD < \begin{proposition}[]\protect\hypertarget{prp-cov_qt}{}\label{prp-cov_qt}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{corollary}[]\protect\hypertarget{cor-cov_qt}{}\label{cor-cov_qt}
\DIFaddend 
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \end{corollary}
\DIFaddend 

\DIFaddbegin \begin{proof}
\DIFadd{See Section~\ref{sec-proof_cov_qt}.
}\end{proof}

\DIFadd{When \(h=1\), Corollary~\ref{cor-cov_qt} reduces to Proposition 1 of
\mbox{%DIFAUXCMD
\citet{angelopoulos2024}}\hskip0pt%DIFAUXCMD
, while Corollary~\ref{cor-cov_qt} extends this
result to a more general multi-step setting and establishes an explicit
upper bound on the coverage gap for multi-step forecasting. The
boundedness assumption on the nonconformity scores is imposed solely for
theoretical analysis, ensuring control over the accumulation of
miscoverage errors in the quantile tracking recursion. When the update
is unraveled, the quantile tracker can be interpreted as an error
integrator that accumulates past deviations of the empirical coverage
from the target level. Bounded scores guarantee the stability of this
accumulation and enable explicit bounds on the finite-sample coverage
gap. Importantly, the bound itself is not required in practice, and the
update proceeds agnostically and remains valid as long as the scores are
finite.
}

\DIFaddend More importantly, Proposition~\ref{prp-cov_rt} is also adequate for
establishing the coverage guarantee of the proposed AcMCP method given
\DIFdelbegin \DIFdel{byEquation }\DIFdelend \DIFaddbegin \DIFadd{by Equation }\DIFaddend \eqref{eq-acmcp}. Following \citet{angelopoulos2024}, we
...
on available historical data. We then present the \DIFaddbegin \DIFadd{long-run }\DIFaddend coverage
guarantee for AcMCP given by Equation \eqref{eq-acmcp_1}.

\DIFdelbegin %DIFDELCMD < \begin{proposition}[]\protect\hypertarget{prp-cov_acmcp}{}\label{prp-cov_acmcp}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{corollary}[]\protect\hypertarget{cor-cov_acmcp}{}\label{cor-cov_acmcp}
\DIFaddend 
...
\DIFaddbegin \begin{proof}
\DIFadd{See Section~\ref{sec-proof_cov_acmcp}.
}\end{proof}

\DIFaddend \section{Experiments}\label{experiments}
...
\item
  ...
  with the largest score seen so far. \DIFaddbegin \DIFadd{For the remaining conformal
  prediction methods, infinitely wide prediction intervals do not arise
  by design, and therefore no clipping is required.
...
%DIFDELCMD < \includegraphics{cpts_files/figure-pdf/fig-AR2_cov-1.pdf}
%DIFDELCMD < 
...
\DIFdelbeginFL \DIFdelFL{black dashed line indicates the }\DIFdelendFL target \DIFaddbeginFL \DIFaddFL{coverage }\DIFaddendFL level \DIFdelbeginFL \DIFdelFL{of }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{is }\DIFaddendFL \(1-\alpha=0.9\).}
...
coverage volatility over time. \DIFdelbegin \DIFdel{The trajectories of }\DIFdelend \DIFaddbegin \DIFadd{For each forecast horizon and method, }\DIFaddend the
rolling mean and median \DIFdelbegin \DIFdel{of the interval widths for each method are largely consistent}\DIFdelend \DIFaddbegin \DIFadd{interval widths follow very similar trajectories
across rolling windows}\DIFaddend . AcMCP constructs narrower prediction intervals
than MPID, despite both methods achieving similar coverage. Moreover, we
see that AcMCP tends to offer \DIFaddbegin \DIFadd{coverage-}\DIFaddend adaptive prediction intervals,
\DIFaddbegin \DIFadd{automatically adjusting their width based on past coverage performance,
}\DIFaddend and results in wider intervals especially when competing methods
...
and MACP. \DIFdelbegin \DIFdel{However, MPID achieves
tight coveragebut at the cost of constructing wider prediction
intervals. This is due }\DIFdelend \DIFaddbegin \DIFadd{Both MPID and AcMCP achieve tight coverage, but AcMCP outputs
more informative and smaller intervals. The benefits of AcMCP are more
noticeable when \(h\) grows, and it does not deteriorate for smaller
\(h\). This improvement can be attributed }\DIFaddend to the inclusion of a second
model (scorecaster) which introduces additional variance into the
generated prediction intervals. The results can be further elucidated
with Figure~\ref{fig-AR2_box}, which presents boxplots of rolling
coverage and interval width for each method and each forecast horizon.
...
%DIFDELCMD < \includegraphics{cpts_files/figure-pdf/fig-AR2_box-1.pdf}
%DIFDELCMD < 
...
%DIFDELCMD < \includegraphics{cpts_files/figure-pdf/fig-AR2_timeplot-1.pdf}
%DIFDELCMD < 
...
presents the prediction interval bounds given by MACP. \DIFaddbegin \DIFadd{Since both MACP
and AcMCP rely on past miscoverage information to update prediction
intervals, we include MACP to support a direct comparison of their
resulting prediction intervals while preserving a clear and concise
presentation. }\DIFaddend The prediction intervals of both AcMCP and MACP can
capture certain patterns in the actual observations, and there is no
consistent pattern indicating dominance of one method over the other in
terms of interval width.
...
%DIFDELCMD < \includegraphics{cpts_files/figure-pdf/fig-NL_cov-1.pdf}
%DIFDELCMD < 
...
%DIFDELCMD < \includegraphics{cpts_files/figure-pdf/fig-NL_box-1.pdf}
%DIFDELCMD < 
...
construct \DIFdelbegin \DIFdel{adaptive prediction intervals }\DIFdelend \DIFaddbegin \DIFadd{prediction intervals that are adaptive to observed changes in
the data distribution}\DIFaddend . They widen intervals in response to undercoverage
...
desired coverage. \DIFdelbegin \DIFdel{AcMCP intervals are evidently
narrower than MPID intervals }\DIFdelend \DIFaddbegin \DIFadd{The outcome of the rolling mean and median interval
widths indicates that AcMCP produces generally narrower prediction
intervals than MPID }\DIFaddend for \(2\)-step-ahead \DIFdelbegin \DIFdel{forecasting but wider }\DIFdelend \DIFaddbegin \DIFadd{forecasts, while producing
wider intervals }\DIFaddend for \(3\)-step-ahead \DIFdelbegin \DIFdel{forecasting. AcMCP intervals appear to be more reasonable, as they tend to widen with increasing forecast horizons}\DIFdelend \DIFaddbegin \DIFadd{forecasts. Furthermore, under
AcMCP, the prediction interval width at horizon \(h=3\) exceeds that at
\(h=2\) in \(53.41\%\) of the test cases, compared with only \(38.08\%\)
under MPID. This pattern suggests that AcMCP may have more plausible
interval behavior, as forecast uncertainty typically increases with the
forecast horizon, leading to a greater tendency for wider prediction
intervals}\DIFaddend .
...
%DIFDELCMD < \includegraphics{cpts_files/figure-pdf/fig-elec_data-1.pdf}
%DIFDELCMD < 
...
\DIFdelend We present the results in Figure~\ref{fig-elec_cov} and
...
for \(h=1,2,3,4\). \DIFaddbegin \DIFadd{While such overcoverage may enhance reliability,
excessively wide prediction intervals reduce forecast sharpness and can
lead to overly conservative operational decisions in electricity
markets, such as inflated reserve procurement and inefficient generation
scheduling. }\DIFaddend Second, MSCP, MWCP, and MACP fail to sustain the target
coverage and noticeably undercover after September 2014 for all forecast
horizons, thus leading to narrower intervals than others. \DIFaddbegin \DIFadd{In electricity
market applications, such undercoverage implies an underestimation of
demand uncertainty, increasing the risk of demand exceeding the
predicted upper bounds and potentially leading to insufficient reserve
allocation and reliability concerns. }\DIFaddend Third, MPI, MPID, and AcMCP offer
prediction intervals that are wider than those of other conformal
prediction methods, effectively mitigating or avoiding the undercoverage
issue observed after September 2014. Additionally, we notice that MPID
performs slightly worse than MPI and AcMCP in terms of coverage for
\DIFdelbegin \DIFdel{\(h=3,4,7\)}\DIFdelend \DIFaddbegin \DIFadd{\(h=3\)}\DIFaddend , despite leading to wider intervals. Finally, MPI and AcMCP
coverage display similar pattern, but AcMCP is capable of constructing
narrower intervals than MPI\DIFdelbegin \DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{We present the forecast interval bounds for MACP, MPI, and AcMCP in
...
intervals are most pronounced in the \(5\)-}\DIFdelend , \DIFdelbegin \DIFdel{\(6\)-, and
\(7\)-step-ahead forecast results. For \(5\)-step-ahead forecasting, from May to July 2014, the upper bounds of MPI intervals struggle to
...
}\DIFdelend \DIFaddbegin \DIFadd{particularly for larger forecast horizons
\(h\). Compared with MPID, the advantage of AcMCP is also reflected in
reduced interval width: its gains become increasingly pronounced as
\(h\) grows, while its performance does not deteriorate for smaller
values of \(h\).
}\DIFaddend 
...
\caption{\label{fig-elec_cov}\DIFaddFL{Electricity demand data results showing
...
\(100\). The target coverage level is \(1-\alpha=0.9\).}}
...
\caption{\label{fig-elec_box}\DIFaddFL{Electricity demand data results showing
...
\DIFaddend \subsection{Eating out expenditure
...
%DIFDELCMD < \includegraphics{cpts_files/figure-pdf/fig-cafe_data-1.pdf}
%DIFDELCMD < 
...
\DIFdelbegin \DIFdel{We summarise the average coverage and interval width }\DIFdelend \DIFaddbegin \DIFadd{As the forecast horizons considered in this application are relatively
...
test set }\DIFaddend for each method and each forecast horizon\DIFdelbegin \DIFdel{in Figure~\ref{fig-cafe_cov}}\DIFdelend . The results first
...
%DIFDELCMD < \includegraphics{cpts_files/figure-pdf/fig-cafe_cov-1.pdf}
%DIFDELCMD < 
...
\DIFaddbegin \subsection{\DIFadd{Post-hoc comparison}}\label{post-hoc-comparison}

\DIFadd{For the MACP method, the step size parameter \(\gamma\) controls both
the calibration tightness and the responsiveness of the prediction
interval widths. Smaller values of \(\gamma\) lead to more gradual
updates of \(\alpha_{t+h|t}\), resulting in smoother temporal variation
in interval lengths. However, this reduced adaptivity also limits the
method's ability to quickly correct local coverage errors, which can
result in larger and more persistent deviations from the nominal
coverage level. Increasing \(\gamma\) enhances the responsiveness of the
local miscoverage rate, but at the cost of less stable interval
evolution. An analogous trade-off arises in AcMCP, where the learning
rate \(\eta\) governs the balance between adaptivity and stability. This
naturally raises the question of whether the empirical advantages of
AcMCP persist when the benchmark MACP method is optimally tuned to be
highly adaptive.
}

\DIFadd{To address this question, we conduct a post-hoc comparison in which the
learning rate \(\eta\) of AcMCP is held fixed, while the step size
\(\gamma\) of MACP is selected via grid search over the interval
\([0.005, 0.5]\). Specifically, \(\gamma\) is chosen to match the sample
variance of local coverage achieved by AcMCP over rolling windows of the
test set.
}

\DIFadd{The results reported in Table~\ref{tbl-posthoc} summarize, for each
forecast horizon, the mean coverage over the full test set, the minimum
and maximum local coverage across rolling windows, and the mean and
median prediction interval widths for the MACP and AcMCP methods across
the data sets considered above. In simple linear and nonlinear
simulation settings, we observe that both methods attain mean coverage
close to the nominal level of 90\%, and AcMCP consistently produces
comparable or narrower prediction intervals when coverage levels are
comparable. The pronounced differences emerge in real-world applications
and at longer forecast horizons. For the electricity demand data, MACP
displays systematic undercoverage as the horizon increases, particularly
in terms of minimum local coverage, whereas AcMCP maintains more stable
local coverage around the target level by adaptively adjusting interval
widths. A similar pattern is observed in the eating out expenditure
data, where MACP suffers from severe coverage degradation at larger
horizons despite tuning, while AcMCP preserves near-nominal coverage at
the cost of wider but informative intervals. These results suggest that
AcMCP offers a substantial improvement over MACP in terms of coverage
control and, when both methods attain coverage close to the target,
AcMCP yields prediction intervals that are shorter or at least
comparable in length.
}

\begin{table}

\caption{\label{tbl-posthoc}\DIFaddFL{Performance of MACP and AcMCP across the four datasets, summarised by mean test-set coverage, minimum and maximum local coverage over rolling windows, and mean and median interval widths for each forecast horizon.}}
...
\DIFaddend \section{Conclusion and discussion}\label{conclusion-and-discussion}
...
The methods discussed here are \DIFdelbegin \DIFdel{limited to }\DIFdelend \DIFaddbegin \DIFadd{restricted to an }\DIFaddend ex-post forecasting
\DIFdelbegin \DIFdel{, operating
under the assumption that actual values of the exogenous predictors during the forecast
period are available. In many applications, this may
not be true and the methods will need to be adapted to allow forecasts
of the
}\DIFdelend \DIFaddbegin \DIFadd{setting, where the values of exogenous }\DIFaddend predictors \DIFdelbegin \DIFdel{to be used, and }\DIFdelend \DIFaddbegin \DIFadd{over the forecast
...
}\emph{\DIFadd{aleatoric uncertainty}}\DIFadd{, arising from the inherent andomness in the
DGP, and }\emph{\DIFadd{epistemic uncertainty}}\DIFadd{, stemming from a lack of knowledge
about the underlying DGP \mbox{%DIFAUXCMD
\citep{sale2025}}\hskip0pt%DIFAUXCMD
. In many practical forecasting
...
propagation of uncertainty from the predictor models to }\DIFaddend the \DIFdelbegin \DIFdel{resulting uncertainty to be
included in the conformal inference}\DIFdelend \DIFaddbegin \DIFadd{primary
...
efficiency}\DIFaddend . Additionally, our methodology does not incorporate an
...
research. \DIFdelbegin \DIFdel{Potential directions include the introduction of a
time-dependent learning rate parameter to minimize interval width while
maintaining the guaranteed coverage rate, as well as the }\DIFdelend \DIFaddbegin \DIFadd{A natural extension of the proposed method is to further
...
intervals that are as narrow as possible. In this work, we restrict
attention to baseline methods that admit efficient online implementation
through simple weighting or updating schemes, and thus are well suited
for multi-step conformal prediction in practice. Methods such as Bellman
conformal inference \mbox{%DIFAUXCMD
\citep{yang2024ts}}\hskip0pt%DIFAUXCMD
, which involve solving an
optimization problem at each time step to explicitly trade off interval
length and miscoverage, are not considered due to their substantially
higher computational cost. Exploring how such optimization-based ideas
can be integrated with the proposed methods to further narrow the
resulting prediction intervals, while preserving coverage guarantees, is
left for future work. Another important direction concerns the
}\DIFaddend development of refined methodologies \DIFdelbegin \DIFdel{that account for variability introduced by
forecasting predictors in ex-ante scenarios}\DIFdelend \DIFaddbegin \DIFadd{for ex-ante forecasting settings,
where uncertainty arising from forecasting the predictors themselves
should be incorporated into the conformal inference framework}\DIFaddend .
...
Proposition~\ref{prp-ar}\DIFdelbegin \DIFdel{; the idea is motivated by \mbox{%DIFAUXCMD
\citet{sommer2023}}\hskip0pt%DIFAUXCMD
}\DIFdelend .
...
\subsection{\DIFdelbegin %DIFDELCMD < \texorpdfstring{Proof of
%DIFDELCMD < Proposition~\ref{prp-cov_qt}}{Proof of Proposition~}%%%
\DIFdelend \DIFaddbegin \texorpdfstring{Proof of
Corollary~\ref{cor-cov_qt}}{Proof of Corollary~}\DIFaddend }\label{sec-proof_cov_qt}
...
\subsection{\DIFdelbegin %DIFDELCMD < \texorpdfstring{Proof of
%DIFDELCMD < Proposition~\ref{prp-cov_acmcp}}{Proof of Proposition~}%%%
\DIFdelend \DIFaddbegin \texorpdfstring{Proof of
Corollary~\ref{cor-cov_acmcp}}{Proof of Corollary~}\DIFaddend }\label{sec-proof_cov_acmcp}
